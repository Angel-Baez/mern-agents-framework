...
function getResultIcon(result) {
  switch (result) {
    case "success":
      return '<i data-lucide="circle-check"></i>';
    case "major":
      return '<i data-lucide="circle-x"></i>';
    case "minor":
...
function getResultBadge(result) {
  ...
  return `<span class="badge ${classes[result] || ""}">${icon} ${
    labels[result] || "Desconocido"
  }</span>`;
}
...
function renderMetrics() {
  ...
  const majorViolations = allCases.filter(
    (c) => getResult(c) === "major"
  ).length;
  const minorViolations = allCases.filter(
    (c) => getResult(c) === "minor"
  ).length;
  const totalViolations = majorViolations + minorViolations;
  const successRate = total > 0 ? ((successes / total) * 100).toFixed(1) : 0;
  const violationRate =
    total > 0 ? ((totalViolations / total) * 100).toFixed(1) : 0;
...
function renderAgentChart() {
  ...
  const agentsWithCases = Object.keys(agentData).filter((agent) => {
    const total =
      agentData[agent].success +
      agentData[agent].major +
      agentData[agent].minor;
...
function renderTable() {
  ...
  if (pageCases.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="6" style="text-align: center; padding: 2rem; color: var(--color-text-secondary);">No se encontraron casos que coincidan con los filtros.</td></tr>';
    return;
  }

  tbody.innerHTML = pageCases
    .map(
      (issue) => `
  ...
  // Reinicializar iconos de Lucide para los elementos dinámicos
...
async function init() {
  ...
  try {
    ...
    if (allCases.length === 0) {
      hideLoading();
      showError(
        'No se encontraron casos de auditoría. Asegúrese de que los issues tienen la etiqueta "audit".'
      );
      return;
    }
